...
export default function Recipes({isFavorite, addToFavorite, removeFromFavorites}){
    ...
    const changeCountry = (newCountry) => {
        axios({
            method : 'GET',
            url: 'https://www.themealdb.com/api/json/v1/1/filter.php?a=' + newCountry
        }).then((response)=>{
    ...
    useEffect(()=>{
        axios({
            ...
            response.data.meals.forEach((item, index) =>{
                if (index == 0)
                    changeCountry(item.strArea)
                setCountries((prev)=> [...prev, item.strArea] )
            })
        })
    }, [])
    useEffect(()=>{
    ...
    return !hasLoaded ?
            ...
            {hasLoaded && <FormControl fullWidth sx={{justifyContent:'center'}}>
                <InputLabel id="Country">Country</InputLabel>
                { country && <Select sx={{height:50}} label-id="Country" label="Country"
